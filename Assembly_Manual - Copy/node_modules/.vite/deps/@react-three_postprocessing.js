import {
  _extends,
  isWebGL2Available
} from "./chunk-L45VQJTT.js";
import {
  useFrame,
  useLoader,
  useThree
} from "./chunk-7APJJXL2.js";
import {
  AlwaysDepth,
  BackSide,
  BasicDepthPacking,
...
var BokehMaterial = class extends ShaderMaterial {
  ...
  generateKernel() {
    ...
    for (let i = 0, sqrt80 = Math.sqrt(80); i < 80; ++i) {
      ...
      const u = r * Math.cos(theta), v32 = r * Math.sin(theta);
      if (i % 5 === 0) {
        points16[i16++] = u;
        points16[i16++] = v32;
      } else {
...
var HueSaturationEffect = class extends Effect {
  ...
  set hue(value) {
    const s = Math.sin(value), c22 = Math.cos(value);
    this.uniforms.get("hue").value.set(
      (2 * c22 + 1) / 3,
      (-Math.sqrt(3) * s - c22 + 1) / 3,
      (Math.sqrt(3) * s - c22 + 1) / 3
    );
  }
  /**
...
var SMAAWeightsMaterial = class extends ShaderMaterial {
   ...
   */
  setLookupTextures(search, area22) {
    this.searchTexture = search;
    this.areaTexture = area22;
  }
  /**
...
`;
    ...
    if (vertexShader2 !== null && /mainSupport/.test(vertexShader2)) {
      ...
      for (const m22 of vertexShader2.matchAll(/(?:varying\s+\w+\s+([\S\s]*?);)/g)) {
        for (const n of m22[1].split(/\s*,\s*/)) {
          data.varyings.add(n);
          varyings.add(n);
          names.add(n);
        }
      }
      for (const m22 of vertexShader2.matchAll(functionRegExp)) {
        names.add(m22[1]);
      }
    }
    for (const m22 of fragmentShader2.matchAll(functionRegExp)) {
      names.add(m22[1]);
    }
...
var Pass2 = class {
  ...
  dispose() {
    for (const key of Object.keys(this)) {
      const property = this[key];
      const isDisposable = property instanceof WebGLRenderTarget || property instanceof Material || property instanceof Texture || property instanceof Pass2;
      if (isDisposable) {
        this[key].dispose();
      }
    }
...
var ClearPass2 = class extends Pass2 {
  constructor(color22 = true, depth = true, stencil = false) {
    super("ClearPass", null, null);
    this.needsSwap = false;
    this.color = color22;
  ...
  render(renderer, inputBuffer, outputBuffer, deltaTime, stencilTest) {
    ...
    renderer.clear(this.color, this.depth, this.stencil);
    if (hasOverrideClearColor) {
      renderer.setClearColor(color2, clearAlpha);
    } else if (hasOverrideClearAlpha) {
      renderer.setClearAlpha(clearAlpha);
    }
  }
...
var AUTO_SIZE2 = -1;
var Resolution2 = class extends EventDispatcher {
  constructor(resizable, width = AUTO_SIZE2, height = AUTO_SIZE2, scale = 1) {
    super();
    this.resizable = resizable;
    this.baseSize = new Vector2(1, 1);
    this.preferredSize = new Vector2(width, height);
  ...
  updateEffectiveSize() {
    ...
    const effective = this.effectiveSize;
    const scale = this.scale;
    if (preferred.width !== AUTO_SIZE2) {
      effective.width = preferred.width;
    } else if (preferred.height !== AUTO_SIZE2) {
      effective.width = Math.round(preferred.height * (base.width / Math.max(base.height, 1)));
    } else {
      effective.width = Math.round(base.width * scale);
    }
    if (preferred.height !== AUTO_SIZE2) {
      effective.height = preferred.height;
    } else if (preferred.width !== AUTO_SIZE2) {
      effective.height = Math.round(preferred.width / Math.max(base.width / Math.max(base.height, 1), 1));
    } else {
      effective.height = Math.round(base.height * scale);
    }
  ...
  set scale(value) {
    if (this.s !== value) {
      this.s = value;
      this.preferredSize.setScalar(AUTO_SIZE2);
      this.dispatchEvent({ type: "change" });
      this.resizable.setSize(this.baseSize.width, this.baseSize.height);
    }
  }
...
var workaroundEnabled2 = false;
var OverrideMaterialManager2 = class {
  constructor(material = null) {
    this.originalMaterials = /* @__PURE__ */ new Map();
    this.material = null;
    this.materials = null;
  ...
  static get workaroundEnabled() {
    return workaroundEnabled2;
  }
  static set workaroundEnabled(value) {
    workaroundEnabled2 = value;
  }
};
var RenderPass2 = class extends Pass2 {
  constructor(scene, camera, overrideMaterial = null) {
    super("RenderPass", scene, camera);
    this.needsSwap = false;
    this.clearPass = new ClearPass2();
    this.overrideMaterialManager = overrideMaterial === null ? null : new OverrideMaterialManager2(overrideMaterial);
    this.ignoreBackground = false;
    this.skipShadowMapUpdate = false;
    this.selection = null;
  }
  ...
  set overrideMaterial(value) {
    ...
    if (value !== null) {
      if (manager !== null) {
        ...
        this.overrideMaterialManager = new OverrideMaterialManager2(value);
      }
    } else if (manager !== null) {
      manager.dispose();
      this.overrideMaterialManager = null;
...
var DepthPass2 = class extends Pass2 {
  constructor(scene, camera, {
    renderTarget,
    resolutionScale = 1,
    ...
    super("DepthPass");
    this.needsSwap = false;
    this.renderPass = new RenderPass2(scene, camera, new MeshDepthMaterial({
      depthPacking: RGBADepthPacking
    }));
    const renderPass = this.renderPass;
    renderPass.skipShadowMapUpdate = true;
    ...
    const resolution = this.resolution = new Resolution2(this, resolutionX, resolutionY, resolutionScale);
    resolution.addEventListener("change", (e) => this.setSize(resolution.baseWidth, resolution.baseHeight));
  }
  set mainScene(value) {
    this.renderPass.mainScene = value;
...
var unpackFactors2 = new Float32Array([
  255 / 256 / 256 ** 3,
  255 / 256 / 256 ** 2,
  255 / 256 / 256,
  255 / 256
]);
var MILLISECONDS_TO_SECONDS2 = 1 / 1e3;
...
var add_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,x+y,opacity);}";
var alpha_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,y,min(y.a,opacity));}";
var average_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,(x+y)*0.5,opacity);}";
var color_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec3 xHSL=RGBToHSL(x.rgb);vec3 yHSL=RGBToHSL(y.rgb);vec3 z=HSLToRGB(vec3(yHSL.rg,xHSL.b));return vec4(mix(x.rgb,z,opacity),y.a);}";
var color_burn_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 z=mix(step(0.0,y)*(1.0-min(vec4(1.0),(1.0-x)/y)),vec4(1.0),step(1.0,x));return mix(x,z,opacity);}";
var color_dodge_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 z=step(0.0,x)*mix(min(vec4(1.0),x/max(1.0-y,1e-9)),vec4(1.0),step(1.0,y));return mix(x,z,opacity);}";
var darken_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,min(x,y),opacity);}";
var difference_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,abs(x-y),opacity);}";
var divide_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,x/max(y,1e-12),opacity);}";
var exclusion_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,(x+y-2.0*x*y),opacity);}";
var hard_light_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 a=min(x,1.0),b=min(y,1.0);vec4 z=mix(2.0*a*b,1.0-2.0*(1.0-a)*(1.0-b),step(0.5,y));return mix(x,z,opacity);}";
var hard_mix_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,step(1.0,x+y),opacity);}";
var hue_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec3 xHSL=RGBToHSL(x.rgb);vec3 yHSL=RGBToHSL(y.rgb);vec3 z=HSLToRGB(vec3(yHSL.r,xHSL.gb));return vec4(mix(x.rgb,z,opacity),y.a);}";
var invert_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,1.0-y,opacity);}";
var invert_rgb_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,y*(1.0-x),opacity);}";
var lighten_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,max(x,y),opacity);}";
var linear_burn_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,clamp(y+x-1.0,0.0,1.0),opacity);}";
var linear_dodge_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,min(x+y,1.0),opacity);}";
var linear_light_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,clamp(2.0*y+x-1.0,0.0,1.0),opacity);}";
var luminosity_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec3 xHSL=RGBToHSL(x.rgb);vec3 yHSL=RGBToHSL(y.rgb);vec3 z=HSLToRGB(vec3(xHSL.rg,yHSL.b));return vec4(mix(x.rgb,z,opacity),y.a);}";
var multiply_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,x*y,opacity);}";
var negation_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,1.0-abs(1.0-x-y),opacity);}";
var normal_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,y,opacity);}";
var overlay_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 z=mix(2.0*y*x,1.0-2.0*(1.0-y)*(1.0-x),step(0.5,x));return mix(x,z,opacity);}";
var pin_light_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 y2=2.0*y;vec4 z=mix(mix(y2,x,step(0.5*x,y)),max(vec4(0.0),y2-1.0),step(x,(y2-1.0)));return mix(x,z,opacity);}";
var reflect_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 z=mix(min(x*x/max(1.0-y,1e-12),1.0),y,step(1.0,y));return mix(x,z,opacity);}";
var saturation_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec3 xHSL=RGBToHSL(x.rgb);vec3 yHSL=RGBToHSL(y.rgb);vec3 z=HSLToRGB(vec3(xHSL.r,yHSL.g,xHSL.b));return vec4(mix(x.rgb,z,opacity),y.a);}";
var screen_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,x+y-min(x*y,1.0),opacity);}";
var soft_light_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 y2=2.0*y;vec4 w=step(0.5,y);vec4 z=mix(x-(1.0-y2)*x*(1.0-x),mix(x+(y2-1.0)*(sqrt(x)-x),x+(y2-1.0)*x*((16.0*x-12.0)*x+3.0),w*(1.0-step(0.25,x))),w);return mix(x,z,opacity);}";
var src_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return y;}";
var subtract_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,max(x+y-1.0,0.0),opacity);}";
var vivid_light_default2 = "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 z=mix(max(1.0-min((1.0-x)/(2.0*y),1.0),0.0),min(x/(2.0*(1.0-y)),1.0),step(0.5,y));return mix(x,z,opacity);}";
var blendFunctions2 = /* @__PURE__ */ new Map([
  [BlendFunction2.ADD, add_default2],
  [BlendFunction2.ALPHA, alpha_default2],
  [BlendFunction2.AVERAGE, average_default2],
  [BlendFunction2.COLOR, color_default2],
  [BlendFunction2.COLOR_BURN, color_burn_default2],
  [BlendFunction2.COLOR_DODGE, color_dodge_default2],
  [BlendFunction2.DARKEN, darken_default2],
  [BlendFunction2.DIFFERENCE, difference_default2],
  [BlendFunction2.DIVIDE, divide_default2],
  [BlendFunction2.DST, null],
  [BlendFunction2.EXCLUSION, exclusion_default2],
  [BlendFunction2.HARD_LIGHT, hard_light_default2],
  [BlendFunction2.HARD_MIX, hard_mix_default2],
  [BlendFunction2.HUE, hue_default2],
  [BlendFunction2.INVERT, invert_default2],
  [BlendFunction2.INVERT_RGB, invert_rgb_default2],
  [BlendFunction2.LIGHTEN, lighten_default2],
  [BlendFunction2.LINEAR_BURN, linear_burn_default2],
  [BlendFunction2.LINEAR_DODGE, linear_dodge_default2],
  [BlendFunction2.LINEAR_LIGHT, linear_light_default2],
  [BlendFunction2.LUMINOSITY, luminosity_default2],
  [BlendFunction2.MULTIPLY, multiply_default2],
  [BlendFunction2.NEGATION, negation_default2],
  [BlendFunction2.NORMAL, normal_default2],
  [BlendFunction2.OVERLAY, overlay_default2],
  [BlendFunction2.PIN_LIGHT, pin_light_default2],
  [BlendFunction2.REFLECT, reflect_default2],
  [BlendFunction2.SATURATION, saturation_default2],
  [BlendFunction2.SCREEN, screen_default2],
  [BlendFunction2.SOFT_LIGHT, soft_light_default2],
  [BlendFunction2.SRC, src_default2],
  [BlendFunction2.SUBTRACT, subtract_default2],
  [BlendFunction2.VIVID_LIGHT, vivid_light_default2]
]);
var BlendMode2 = class extends EventDispatcher {
  constructor(blendFunction, opacity = 1) {
    super();
    this._blendFunction = blendFunction;
    this.opacity = new Uniform(opacity);
  ...
  getShaderCode() {
    return blendFunctions2.get(this.blendFunction);
  }
};
var Effect2 = class extends EventDispatcher {
  constructor(name, fragmentShader2, {
    attributes = EffectAttribute2.NONE,
    ...
    this.uniforms = uniforms;
    this.extensions = extensions;
    this.blendMode = new BlendMode2(blendFunction);
    this.blendMode.addEventListener("change", (event) => this.setChanged());
...
var orthogonalSubsamplingOffsets2 = new Float32Array([
  0,
  -0.25,
  0.25,
  -0.125,
...
var diagonalSubsamplingOffsets2 = [
  new Float32Array([0, 0]),
  new Float32Array([0.25, -0.25]),
  new Float32Array([-0.25, 0.25]),
  new Float32Array([0.125, -0.125]),
...
var edges2 = /* @__PURE__ */ new Map([
  [bilinear2(0, 0, 0, 0), new Float32Array([0, 0, 0, 0])],
  [bilinear2(0, 0, 0, 1), new Float32Array([0, 0, 0, 1])],
  [bilinear2(0, 0, 1, 0), new Float32Array([0, 0, 1, 0])],
  [bilinear2(0, 0, 1, 1), new Float32Array([0, 0, 1, 1])],
  [bilinear2(0, 1, 0, 0), new Float32Array([0, 1, 0, 0])],
  [bilinear2(0, 1, 0, 1), new Float32Array([0, 1, 0, 1])],
  [bilinear2(0, 1, 1, 0), new Float32Array([0, 1, 1, 0])],
  [bilinear2(0, 1, 1, 1), new Float32Array([0, 1, 1, 1])],
  [bilinear2(1, 0, 0, 0), new Float32Array([1, 0, 0, 0])],
  [bilinear2(1, 0, 0, 1), new Float32Array([1, 0, 0, 1])],
  [bilinear2(1, 0, 1, 0), new Float32Array([1, 0, 1, 0])],
  [bilinear2(1, 0, 1, 1), new Float32Array([1, 0, 1, 1])],
  [bilinear2(1, 1, 0, 0), new Float32Array([1, 1, 0, 0])],
  [bilinear2(1, 1, 0, 1), new Float32Array([1, 1, 0, 1])],
  [bilinear2(1, 1, 1, 0), new Float32Array([1, 1, 1, 0])],
  [bilinear2(1, 1, 1, 1), new Float32Array([1, 1, 1, 1])]
]);
function lerp22(a, b, p) {
  return a + (b - a) * p;
}
function bilinear2(e0, e1, e2, e3) {
  const a = lerp22(e0, e1, 1 - 0.25);
  const b = lerp22(e2, e3, 1 - 0.25);
  return lerp22(a, b, 1 - 0.125);
}
...
var ReflectionsPass = class extends Pass2 {
  constructor(ssrEffect, options = {}) {
    ...
    this.renderPass = new RenderPass2(this._scene, this._camera);
    ...
    if (this.USE_MRT) {
      ...
      this.webgl1DepthPass = new DepthPass2(this._scene, this._camera);
  ...
  setMRTMaterialInScene() {
    ...
    for (const c3 of this.visibleMeshes) {
      if (c3.material) {
        const originalMaterial = c3.material;
        let [cachedOriginalMaterial, mrtMaterial] = this.cachedMaterials.get(c3) || [];
        if (originalMaterial !== cachedOriginalMaterial) {
          ...
          this.cachedMaterials.set(c3, [originalMaterial, mrtMaterial]);
        }
        ...
        mrtMaterial.uniforms.roughness.value = this.ssrEffect.selection.size === 0 || this.ssrEffect.selection.has(c3) ? originalMaterial.roughness || 0 : 1e11;
        c3.material = mrtMaterial;
      }
    }
  }
  unsetMRTMaterialInScene() {
    for (const c3 of this.visibleMeshes) {
      var _c$material;
      if (((_c$material = c3.material) == null ? void 0 : _c$material.type) === "MRTMaterial") {
        c3.visible = true;
        const [originalMaterial] = this.cachedMaterials.get(c3);
        c3.material = originalMaterial;
      }
    }
  }
...
var VelocityPass = class extends Pass2 {
  constructor(scene, camera) {
  ...
  setVelocityMaterialInScene() {
    ...
    for (const c3 of this.visibleMeshes) {
      var _c$skeleton2;
      const originalMaterial = c3.material;
      let [cachedOriginalMaterial, velocityMaterial] = this.cachedMaterials.get(c3) || [];
      if (originalMaterial !== cachedOriginalMaterial) {
        var _c$skeleton;
        velocityMaterial = new VelocityMaterial();
        velocityMaterial.lastMatrixWorld = new Matrix4();
        c3.material = velocityMaterial;
        if ((_c$skeleton = c3.skeleton) != null && _c$skeleton.boneTexture)
          this.saveBoneTexture(c3);
        this.cachedMaterials.set(c3, [originalMaterial, velocityMaterial]);
      }
      velocityMaterial.uniforms.velocityMatrix.value.multiplyMatrices(this._camera.projectionMatrix, c3.modelViewMatrix);
      if (c3.userData.needsUpdatedReflections || originalMaterial.map instanceof VideoTexture) {
      ...
      c3.visible = this.cameraMovedThisFrame || !c3.matrixWorld.equals(velocityMaterial.lastMatrixWorld) || c3.skeleton || "FULL_MOVEMENT" in velocityMaterial.defines;
      c3.material = velocityMaterial;
      if (!c3.visible)
        continue;
      ...
      if ((_c$skeleton2 = c3.skeleton) != null && _c$skeleton2.boneTexture) {
        velocityMaterial.defines.USE_SKINNING = "";
        velocityMaterial.defines.BONE_TEXTURE = "";
        velocityMaterial.uniforms.boneTexture.value = c3.skeleton.boneTexture;
      }
    }
  }
  ...
  unsetVelocityMaterialInScene() {
    for (const c3 of this.visibleMeshes) {
      if (c3.material.isVelocityMaterial) {
        var _c$skeleton3;
        c3.visible = true;
        c3.material.lastMatrixWorld.copy(c3.matrixWorld);
        c3.material.uniforms.prevVelocityMatrix.value.multiplyMatrices(this._camera.projectionMatrix, c3.modelViewMatrix);
        if ((_c$skeleton3 = c3.skeleton) != null && _c$skeleton3.boneTexture)
          this.saveBoneTexture(c3);
        c3.material = this.cachedMaterials.get(c3)[0];
      }
    }
  }
  setSize(width, height) {
...
var TemporalResolvePass = class extends Pass2 {
...
var pmremGenerator;
var SSREffect = class extends Effect2 {
  /**
  ...
  setIBLRadiance(iblRadiance, renderer) {
    this._scene.traverse((c3) => {
      if (c3.material) {
        var _renderer$properties$;
        const uniforms = (_renderer$properties$ = renderer.properties.get(c3.material)) == null ? void 0 : _renderer$properties$.uniforms;
  ...
  update(renderer, inputBuffer) {
    if (!this.usingBoxProjectedEnvMap && this._scene.environment) {
      ...
      let envMap = null;
      this._scene.traverse((c3) => {
        if (!envMap && c3.material && !c3.material.envMap) {
          const properties = renderer.properties.get(c3.material);
